...
const MUTE_KEY = "insider:musicMuted";
...
let currentTimerSnapshot = null; // { startAt, durationMs, serverNow } while the guessing clock runs
...
// A rhythmic pulse cycling through a short note pattern — used for the guessing countdown.
...
// A single resolving chord, not looped — for role-reveal and the results screen.
...
function sceneKeyForPhase(phase) {
  if (phase === "guessing") return "tension";
  if (phase === "voting") return "suspense";
  return "ambient"; // landing, lobby, role-reveal, results
}
...
export function updateForState(state, { serverNow } = {}) {
  ...
  currentTimerSnapshot =
    activePhase === "guessing" && state.public?.timer && serverNow
  ...
  if (activePhase === "results") playSting([261.63, 329.63, 392.0, 523.25], { duration: 2.0 }); // resolving chord
